...
public class PaymentSummaryConsumer {
    public static void main(String[] args) {
        ...
        consumer.subscribe(Pattern.compile("payments.summary"));

        Gson gson = new Gson();
        System.out.println("👂 Listening on all payment topics...");

        while (true) {
            ConsumerRecords<String, String> records = consumer.poll(Duration.ofMillis(1000));
            for (ConsumerRecord<String, String> record : records) {
            	System.out.printf("MERGED>>>>>>>>>" + record.value());
                Payment payment = gson.fromJson(record.value(), Payment.class);
                System.out.printf("[Topic: %s | Partition: %d | Offset: %d]%nKey: %s%nValue: %s%n%n",
